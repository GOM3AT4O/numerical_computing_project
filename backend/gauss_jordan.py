...
class GaussJordanSolver(LinearSystemSolver):
    ...
    # Additional method for scaled partial pivoting
    def solve_using_scaling(self) -> SolutionResult:
        start_time = time.time()
        ...
        scale_factors = self.Calculating_Scaling_values(A)

        for k in range(n):
            # Scaling pivoting
            A, b = self.Scaling_Partial_Pivot(A,b,scale_factors,k)
