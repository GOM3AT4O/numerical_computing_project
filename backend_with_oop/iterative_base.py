...
class IterativeSolver(LinearSystemSolver):
    def __init__(self, A: np.ndarray, b: np.ndarray, precision: int = 6,
        ...
        if max_iterations is not None and tolerance is not None:
            raise ValidationError("can't specify both max_iterations and tolerance. kindly specify only one thing don't get me comfused.")
        ...
        if initial_guess is not None:
            ...
            if len(self.x0)!=self.n:
                raise ValidationError(f"initial guess length must match number of variables ({self.n})")
        else:
    ...
    def analyze_system(self)->Optional[str]:
        ...
        if zero_diagonals:
            ...
            return f"can't use iterative methods: zero diagonal elements found in equations {zero_diagonals}. kindly reorder your equations to avoid zero diagonal elements."
